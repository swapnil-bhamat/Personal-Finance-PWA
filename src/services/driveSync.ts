...
export async function initializeFromDrive(shouldRestore = false): Promise<boolean> {
  try {
    if (shouldRestore) {
    ...
    await uploadJsonFile(currentData, DATA_FILE_NAME);
    logInfo('Created new data file in Google Drive');
    return true;
  } catch (error) {

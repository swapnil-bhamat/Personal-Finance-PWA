...
// Mock Utils
vi.mock('@/shared/utils/numberUtils', () => ({
  toLocalCurrency: (val: number) => `₹${val}`
}));

// Mock child components to avoid deep rendering issues or missing contexts
vi.mock('@/shared/components/DailyTipCard', () => ({ default: () => null }));
vi.mock('@/shared/components/GoldRateCard', () => ({ default: () => null }));
vi.mock('@/domains/budgets/components/GoalProgressChart', () => ({ default: () => null }));
vi.mock('@/shared/components/Gauge', () => ({ default: () => null }));
vi.mock('@/domains/analytics/components/CashFlowDiagram', () => ({ default: () => null }));
